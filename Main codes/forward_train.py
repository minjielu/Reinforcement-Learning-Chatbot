...
import time
from BuildModel import Seq2Seq
...
target_sequence_length, MLE_optimizer, MLE_loss, \
training_logits = Seq2Seq().build_model()
...
with tf.Session() as sess:
    ...
    if start_new:
        ...
        saver.restore(sess, "./models/forward_model")
    epochs = 2
    loss_acc_log = []
    ...
    for epoch_i in np.arange(epochs):
        ...
        bg1 = batch_generator('Inputs/', batch_size)
        bg2 = batch_generator('Outputs/', batch_size)
        ...
        while over_1 and over_2:
            ...
            if batch_num % 1 == 0:
                ...
                saver = tf.train.Saver()
                saver.save(sess, './models/forward_model_1')
